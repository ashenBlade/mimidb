...
UndoLog *UndoLog::Open(std::string path) {
    auto file = io::File::Open(path, O_RDONLY | O_CREAT);
    auto size = file.Size();
    return new UndoLog{path, size};
}

static UndoLogRecordHeader read_header(mi::storage::io::File &file, off64_t offset) {
...
std::unique_ptr<IUndoRecord> UndoLog::GetRecord(UndoSeqNumber usn) {
    ...
    auto file = io::File::Open(this->_path, O_RDONLY);

...
InsertUndoRecord::InsertUndoRecord(Oid tableId, TupleId location, std::vector<std::byte> tupleData)
    : HeapUndoRecord(HeapUndoRecordType::Insert), TableId(tableId), Location(location),
...
void InsertUndoRecord::Accept(IHeapUndoRecordVisitor &visitor) { visitor.Visit(*this); }

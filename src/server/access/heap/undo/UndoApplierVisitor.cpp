...
void UndoApplierVisitor::Visit(UpdateUndoRecord &record) {
    if (record.NewLocation == record.OldLocation) {
        ...
        // Someone concurrently updated tuple. This is invalid, since nobody can do it - protocol
        // violation.
        ...
        std::memcpy(reinterpret_cast<std::byte *>(currentTuple), record.TupleData.data(),
        ...
        // Actually rewrite tuple to old data. Do not touch ItemId since it wasn't modified.
        std::memcpy(reinterpret_cast<std::byte *>(currentOldTuple), record.TupleData.data(),
                    record.TupleData.size());
